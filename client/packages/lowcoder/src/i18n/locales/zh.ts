...
textOverflowProp: {// added by Mousheng
    ellipsis: "省略号",
    wrap: "换行"
},
labelProp: {
...
style: {
    ...
    aspectRatio: "纵横比",// added by Mousheng
},
export: {
...
uiCompCategory: {// added by Mousheng
    dashboards: "仪表盘与图表",
    layout: "布局与导航",
    forms: "数据收集与表单",
    collaboration: "会议与协作",
...
customComp: {
    ...
    data: "要传递给自定义组件的数据",// added by Mousheng
    code: "自定义组件的代码",
},
tree: {
    selectType: "选择类型",
...
richTextEditor: {
    ...
    edit: "编辑",
    remove: "移除",
    toolbar: "自定义工具栏",// added by Mousheng
...
date: {
    ...
    startDate: "开始日期",
    endDate: "结束日期",
    placeholderText: "占位符",  // added by Mousheng
},
time: {
    start: "开始时间",
    end: "结束时间",
...
table: {
    ...
    rowHeight: "条件行高",//added by Mousheng
    rowHeightDesc: "根据可选变量（CurrentRow、CurrentOriginalIndex、CurrentIndex、ColumnTitle）有条件地设置行高。例如: '{{ currentRow.id > 3 ? \"60px\" : \"40px\" }}'",
},
image: {
    src: "图片链接",
    srcDesc: "图片链接地址",
...
meeting: {  // added by Mousheng
    ...
    meetingCompKeywords: "Agora 会议，Web 会议，协作",
    iconSize: "图标大小",
    userId: "主持人用户 ID",
    roomId: "房间 ID",
...
transformer: {
    ...
    documentationText: "Transformers旨在进行数据转换和重用多行JavaScript代码。使用Transformers将查询或组件的数据适应到本地应用程序的需求。与JavaScript查询不同，transformer设计为执行只读操作，这意味着您无法在transformer内触发查询或更新临时状态。",//added by mousheng
},
temporaryState: {
    value: "初始值",
    ...
    documentationText: "低代码中的临时状态是一个强大的功能，用于管理动态更新应用程序中组件状态的复杂变量。这些状态充当数据的中间或瞬时存储，可以由用户交互或其他过程引起的时间变化。", //added by Mousheng
},
dataResponder: {
    data: "数据",
    dataDesc: "当前数据响应器的数据",
    ...
    documentationText: "在开发应用程序时，您可以为组件分配事件以监视特定数据的更改。例如，Table 组件可能具有事件如“行选择更改”、“筛选更改”、“排序更改”和“分页更改”，以跟踪选定行属性的变化。然而，对于临时状态、转换器或查询结果的更改，其中没有标准事件可用，将使用数据响应器。它们使您能够检测和响应任何数据修改。",//added by mousheng
},
theme: {
    title: "主题",
    createTheme: "创建主题",
...
module: {
    ...
    emptyEvent: "尚未添加事件",
    event: "事件",
    docLink: "了解更多关于模块的信息...",//added by mousheng
...
aggregation: {
    ...
    selectedKey: "当前选定的索引", //added by mousheng
},
appSetting: {
    title: "应用设置",
    pageSetting: "页面设置",
...
header: {
    ...
    clone: "克隆",
    editorMode_layout: "布局",  //added by mousheng
    editorMode_logic: "逻辑",
    editorMode_both: "布局和逻辑",
},
userAuth: {
...
queryTutorial: {
    ...
    dataResponder: "https://docs.lowcoder.cloud/lowcoder-documentation/business-logic-in-apps/write-javascript/data-responder",//added by mousheng
},
customComponent: {
...
idSource: {
    ...
    timeLine: {
        ...
        scrollTo: "控制滚动条滚动",
        pendingDescription: "当设置时，将显示一个带有文本和等待指示器的最后一个节点",//added by mousheng
    },
    jsonLottie: {
        lottieJson: "JSON数据",
        speed: "播放速度",
